...
public:
	...
	void set_num() {		//숫자 세팅
		num = _getch();		//글자받기
		//if (num == 32)		//지금은 32(스페이스 누를시 종료 ㅠ
			//exit(1);
	}
	void print_num() {
		cout << num << endl;
...
public:
	Saveinarray() {
		s = new Returnvalue[6];
		array[0] = s[0].get_value(); array[1] = s[1].get_value();
		array[2] = s[2].get_value(); array[3] = s[3].get_value();
		array[4] = s[4].get_value(); array[5] = s[5].get_value();
